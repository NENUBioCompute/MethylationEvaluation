...
#### NO.03 Zhang Clocks ####
ZhangAge <- function (beta_path, fill_value) {
  ...
  read.table("R/03/blup.coef",stringsAsFactor=F,header=T)->blupcoef
  ...
  read.table("R/03/en.coef",stringsAsFactor=F,header=T)->encoef
  read.table("R/03/blup.coef",stringsAsFactor=F,header=T)->blupcoef
