...
Horvath <- function(beta_path, fill_value) {
  library("data.table")
  ...
  source("R/01/NORMALIZATION.R")
  trafo= function(x,adult.age=20) { x<-(x+1)/(1+adult.age); y<-ifelse(x<=1, log( x),x-1);y }
  anti.trafo= function(x,adult.age=20) { ifelse(x<0, (1+adult.age)*exp(x)-1, (1+adult.age)*x+adult.age) }
  probeAnnotation21kdatMethUsed=read.csv("R/01/probeAnnotation21kdatMethUsed.csv")
  probeAnnotation27k=read.csv("R/01/datMiniAnnotation27k.csv")
  datClock=read.csv("R/01/AdditionalFile3.csv")
  ...
  # create log data
  file.remove("R/01/LogFile.txt")
  file.create("R/01/LogFile.txt")
  DoNotProceed=FALSE
