...
CorticalClock <- function(beta_path, fill_value) {
  ...
  braincoef<-read.table("R/24/CorticalClockCoefs.txt", stringsAsFactor=F,header=T)
  ...
  if (length(overlap) < nrow(braincoef)) {
    ...
    load("R/24/Ref_DNAm_brain_values.rdat")
